...
metadata {
	definition (
		...
		fingerprint mfr:"027A", prod:"0004", deviceId:"0110", inClusters:"0x00,0x00", controllerType: "ZWV" //Zooz ZEN55 DC Signal Sensor
	}
	...
	preferences {
		...
		input "healthCheckInterval", "enum", title: fmtTitle("Healthcheck Interval"),
			description: fmtDesc("How often the hub will check the device health by sending a refresh"),
			options: [0: "Disabled", 15: 'Every 15 Mins', 30: 'Every 30 Mins', 60: 'Every 1 Hour', 240: 'Every 3 Hours', 360: 'Every 6 Hours', 720: 'Every 12 Hours'],
			defaultValue: 720, required: true
	}
}
...
[
	...
	configRelaySensor: [ num: 8,
		...
		options: [
			...
			4:"Enable Smoke, CO, and the Relay [ALL]",
...
void parse(String description) {
	...
	if (healthCheckInt) sendEvent(name:"healthStatus", value:"online")
...
void executeRefreshCmds() {
	...
	cmds << notificationGetCmd(0x02, 0x02)

	if (healthCheckInt) runIn(10, healthCheckTimeout)
...
********************************************************************/
//Health Check
Integer getHealthCheckInt() {
	return (healthCheckInterval as Integer)
}

void scheduleHealthCheck() {
	Integer hci = healthCheckInt
	Random rnd = new Random()

	if (hci >= 60) {
		Integer hrs = hci / 60
		schedule("${rnd.nextInt(59)} ${rnd.nextInt(59)} 0/${hrs} * * ?", 'healthCheck')
	}
	else if (hci > 0) {
		schedule("${rnd.nextInt(59)} ${rnd.nextInt(hci-1)}/${hci} * * * ?", 'healthCheck')
	}
...
void healthCheck() {
	if (healthCheckInt > 0) {
		logInfo("Running Device Health Check")
		executeRefreshCmds()
	} else {
...
void healthCheckTimeout() {
	logWarn("Device did not respond, healthStatus set to offline")
