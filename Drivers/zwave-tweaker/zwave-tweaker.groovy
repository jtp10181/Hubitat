 ...
 *    the Apache 2.0 License from the original work of *David Lomas*.
 *
 *  Copyright: David Lomas (codersaur)
 *  Copyright: 2020 Jeff Page (jtp10181)
 *
 *  Name: Z-Wave Tweaker
 *
...
def zwaveEvent(hubitat.zwave.commands.supervisionv1.SupervisionGet cmd) {
    logger("zwaveEvent(): Supervision Get command received: ${cmd}","info")

    def encapsulatedCommand = cmd.encapsulatedCommand(getCommandClassVersions())
    if (!encapsulatedCommand) {
        logger("zwaveEvent(): Could not extract command from ${cmd}","error")
    } else {
        cacheCommandMd(encapsulatedCommand, "SUPERVISION_GET")
        return zwaveEvent(encapsulatedCommand)
    }

    sendCommands(zwave.supervisionV1.supervisionReport(sessionID: cmd.sessionID, reserved: 0, moreStatusUpdates: false, status: 0xFF, duration: 0))
}
...
def zwaveEvent(hubitat.zwave.commands.multichannelassociationv2.MultiChannelAssociationReport cmd) {
    ...
    List<Integer> mcList = []
    List<String> mcListHex = []
    cmd.multiChannelNodeIds.each { 
        mcList << it.nodeId << it.endPointId
        mcListHex << "${toHexString(it.nodeId)}:${toHexString(it.endPointId)}"
    }
    if (mcList) {mcList.add(0,00)} //Add 00 Spacer first for multichannel

    logger("Association Group #${cmd.groupingIdentifier} contains destinations: ${toHexString(cmd.nodeId)} ${mcListHex} (hexadecimal format)","info")
...
def zwaveEvent(hubitat.zwave.commands.security2v1.Security2CommandsSupportedReport cmd) {
    logger("zwaveEvent(): Security2 Commands Supported Report received: ${cmd}","trace")

    state.useSecurity = true
    state.zwtGeneralMd.securityCommandClassSupport = cmd.commandClasses.sort().collect{ it as Integer }
    //state.zwtGeneralMd.securityCommandClassControl = cmd.commandClassControl.sort()
...
private initialise() {
    ...
    List ccIds = []
    ccIds = getDataValue("inClusters")?.split(",").collect()
    ccIds += getDataValue("secureInClusters")?.split(",").collect()
    ...
    state.zwtGeneralMd.commandClassIds = ccIds.sort().collect { hexStrToUnsignedInt(it) as Integer } // Parse hex strings to ints.
    ...
    cmds << zwave.security2V1.security2CommandsSupportedGet()
    //cmds << "9F0D" //SECURITY_2_COMMANDS_SUPPORTED_GET
...
private scanActuator() {
    ...
    cmds << zwave.switchColorV3.switchColorSupportedGet()
...
private scanSensor() {
    ...
    if (cmds) {
        ...
        logger("scanSensor(): NO SENSOR COMMAND CLASSES","warn")
    }
}
...
private cleanUp() {
    ...
    device?.removeSetting("zwtLoggingLevelIDE")
    device?.removeSetting("zwtAssocGroupsScanStart")
    device?.removeSetting("zwtAssocGroupsScanStop")
    device?.removeSetting("zwtEndpointsScanStart")
    device?.removeSetting("zwtEndpointsScanStop")
    device?.removeSetting("zwtParamsScanStart")
    device?.removeSetting("zwtParamsScanStop")
    device?.removeSetting("zwtAssocGroupId")
    device?.removeSetting("zwtAssocGroupMembers")
    device?.removeSetting("zwtAssocGroupCc")
    device?.removeSetting("zwtParamId")
    device?.removeSetting("zwtParamValue")
    device?.removeSetting("zwtProtectLocal")
    device?.removeSetting("zwtProtectRF")
    device?.removeSetting("zwtSwitchAllMode")

    logger("states: {$state}","info")
    logger("settings: {$settings}","info")
}
...
private encapCommand(String cmd) {
    return zwaveSecureEncap(cmd)
}
...
// }
private sendCommands(List<String> cmds, Long delay=400) {
    sendHubCommand(new hubitat.device.HubMultiAction(delayBetween(cmds.collect{ encapCommand(it) }, delay), hubitat.device.Protocol.ZWAVE))
}

private sendCommands(String cmd) {
    sendHubCommand(new hubitat.device.HubAction(encapCommand(cmd), hubitat.device.Protocol.ZWAVE))
}
...
private updateSyncPending() {
    ...
    if (state.zwtAssocGroupTarget  != null) { // There's an association group target to sync.
        ...
        //log.debug "${cachedNodes} | ${targetNodes}"
